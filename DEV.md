...
Generated-site qualification is the standard path for eligible landing and marketing builds with an accepted handoff; `interactive_app` remains on its existing implementation-spec path. `quality.generated_site_critic_sample_rate` is stored as `0..1`, displayed as a percentage in admin, and uses deterministic sampling, never `Math.random()`. Discuss moderation starts concurrently with safe request preparation but must pass before the discuss turn starts; partial structured tool output always streams best-effort. Secrets, OAuth, DB/S3 URLs, and topology stay env-only and still need an app restart when changed.
