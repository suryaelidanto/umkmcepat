...
Generated-site quality has two paths: the deterministic renderer remains the named control/rollback path, while reference-calibrated V2 is disabled by default. Settings `feature.reference_calibrated_generation_enabled=false` and `feature.reference_calibrated_generation_shadow=true` resolve to `off`; enabling it first resolves to `shadow`. Replacement is downgraded to shadow until code-owned benchmark approval exists. Shadow proof/evidence cannot replace source, deployment, accepted handoff, or charge the owner success path twice. The V2 call ledger is exactly one writer, one critic, and at most one shared correction; all model calls use `maxRetries: 0`. `interactive_app` remains on its existing implementation-spec path. The old `quality.generated_site_critic_sample_rate` setting is legacy/control telemetry only; V2 always performs one visual review. Discuss moderation starts concurrently with safe request preparation but must pass before the discuss turn starts; partial structured tool output always streams best-effort. Secrets, OAuth, DB/S3 URLs, and topology stay env-only and still need an app restart when changed.
