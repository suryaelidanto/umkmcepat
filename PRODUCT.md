...
The platform is 100% free for users — every feature is usable without paying. Access is managed via a pilot whitelist with admin approval (initially capped at ~10 active UMKM businesses). Users can optionally buy an Energy Booster (paid, non-expiring extra energy) via a modal in the navbar profile dropdown when the free daily quota runs out; paying never gates features, so the core stays free.
