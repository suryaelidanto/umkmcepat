...
- Discussion turns use one visible streaming AI call for chat text, followed by a hidden schema-validated call for structured workspace UI. The hidden card stage has its own short timeout, bounded semantic repair, and an outer server deadline; the visible reply is never regenerated just to repair its card. The route duration must cover the normal discussion timeout plus this bounded repair window.
