...
- Workspace cards are never parsed from chat text. A missing or failed tool output must never overwrite the last valid card with `none`; once its question is answered, that card is stale and must be hidden immediately. The client shows an explicit retry action that repairs only the hidden next card—never replays the answer or streams a second visible reply—if both the primary tool call and bounded repair fail.
...
- Generated frontend apps send a `umkmcepat-preview-ready` postMessage after React renders. The workspace keeps its loading state while the ready signal is absent and performs bounded silent recovery for cold starts; it must not ask users to reload a preview that can still start itself. Only a terminal build failure without a successful artifact becomes an actionable error state.
