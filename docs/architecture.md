...
- Workspace cards are never parsed from chat text. If a tool call advances the workspace card before the stream finishes, the server also persists a provisional assistant tool turn so refreshes never show a card/chat mismatch. If the tool output is missing, the client shows an explicit retry state instead of fabricating a fallback card.
