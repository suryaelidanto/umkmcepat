...
- The workspace has a designed mobile layout (not a responsive-desktop layout): a sticky bottom nav (`md:hidden`) surfaces Diskusi / Tampilan / Kode as single-surface swaps with `min-h-11` touch targets and `env(safe-area-inset-bottom)` padding; the root uses `h-dvh` for mobile-browser-chrome correctness; the Code view's file tree caps to `38dvh` on mobile (content gets the rest) and restores its 280px desktop column at `md:`. The visual-annotation editor (pointer-driven) is desktop-only — mobile users edit via chat, not pixel-annotation; do not ship pointer annotation on touch. Every workspace state (loading, ready, stuck, build-failed, runtime-stopped, rate-limited) must read legibly on mobile.
- Build generation asks AI for a schema-validated flexible implementation spec before writing source. The spec lets AI choose landing, marketing site, or interactive full-stack app shape, including pages, components, features, content, and visual direction. A landing page is one valid outcome, not the forced default. Invalid or incomplete specs receive one bounded semantic retry, then fail the build honestly before source generation; deterministic compatibility specs must never replace a failed AI result. New project rows do not pre-populate generic site content. Legacy site schema remains compatibility data for existing records only; it must never be presented as successful AI-generated final source. The spec also carries an `archetype` id; the spec-call system prompt appends a compact archetype index and a rule forbidding the default Hero→Fitur→Testimoni→Kontak skeleton unless each section is justified, and the source-gen prompt loads the matching guidance doc from `src/lib/projects/archetypes/` (one per business shape, `generic-fallback.md` as a decision-framework fallback) so structure diverges per business instead of stamping the same template. `KNOWN_ARCHETYPE_IDS` lists all valid ids independent of authored docs; `loadArchetypeGuide` falls back to the generic doc for any id without a guide yet.
