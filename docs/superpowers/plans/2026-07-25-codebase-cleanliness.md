...
- Graphify: binary not on PATH here → fall back to Explore agents + stale `graphify-out/`; do NOT add Graphify as a project dependency (per `CLAUDE.md`).
...
- [ ] **Step 1: Walk recent-commit hot spots**
...
- [ ] **Step 2: Invoke the improve-codebase-architecture skill**
...
- [ ] **Step 3: Render the candidate HTML report**
...
- [ ] **Step 4: Open the report for the user + pick candidates**
...
- [ ] **Step 5: No commit** (discovery output lives in OS temp; nothing lands in the repo yet).
...
- [ ] **Step 1: Confirm the gate is green before starting**

Run: `bun run check`
Expected: green (baseline).

- [ ] **Step 2: Sweep a directory chunk for obvious comments**
...
- [ ] **Step 3: Run the gate after each chunk**
...
- [ ] **Step 5: Repeat** for each directory chunk until the sweep is complete.
...
- [ ] **Step 1: Order candidates by risk (lowest-risk, highest-leverage first)**
...
- [ ] **Step 6: Repeat** for each picked candidate, each its own atomic commit. Stop when a candidate can't be proven behavior-preserving — defer it (note in the discovery report).
...
- [ ] **Step 1: Add the Cleanliness contract section to DEV.md**
...
- [ ] **Step 2: Confirm no behavior change** — the test suite passing is the proof; spot-check a generation + edit + publish flow manually if the refactors touched those paths.
