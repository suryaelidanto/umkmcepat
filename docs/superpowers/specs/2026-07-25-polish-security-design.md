...
**Status:** Mostly shipped — plan complete (`d7107a7`); dark chrome, `mapToUserFacingError` on the 4 leak sites, JSON-LD `@graph` (no `aggregateRating`), locked homepage copy, sitemap enumerating `/p/<slug>`, Umami `track()`, OTP crypto, waitlist magic-byte, `runtime-proxy.ts` postMessage origin-targeting. **Decision 8 (PATH-narrow + frozen-lockfile): resolved as moot 2026-07-26** — see decision 8 below; the `--ignore-scripts` + `BLOCKED_GENERATED_PATHS` (lockfiles blocked) + package-policy lifecycle blocking already cover the threat; shipping them would break builds with no security gain.
...
8. **Generated-site sandbox hardening (theoretical, harden now):**
   ...
   **RESOLVED 2026-07-26 — both sub-items moot, not shipped (intentionally):** the build child runs `bun install --ignore-scripts` (generated-source.ts:373) — `--ignore-scripts` is the real supply-chain defense, and `generated-package-policy.ts` re-blocks lifecycle scripts (postinstall/preinstall/prepare) + `BLOCKED_PACKAGE_FIELDS` at the package.json validation layer. PATH-narrowing adds no defense because no install scripts execute to resolve leaked PATH binaries; the child only runs `vite build` via the resolved absolute `BUNDLED_RUNNER` (`process.execPath`), whose own spawns (esbuild/rollup) resolve from `node_modules` (already-installed, trusted). `--frozen-lockfile` is moot AND would break builds: `bun.lock`/`bun.lockb`/`package-lock.json` are in `BLOCKED_GENERATED_PATHS` (generated-source.ts:62-68) — the AI cannot emit a lockfile, so there is nothing to freeze against; `--frozen-lockfile` would fail every install. No security gap remains; the standing defenses cover the intended threat.
