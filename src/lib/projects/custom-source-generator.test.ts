...
describe("cssCoversClassName (meaningful rule semantics)", () => {
  ...
  it("treats Tailwind v4 order + tabular-nums utilities as covered (regression: legacy loop)", () => {
    // order-1/order-2 and tabular-nums are standard Tailwind v4 utilities
    // generated by @tailwindcss/vite; flagging them as missing CSS forced
    // expensive legacy agent rewrites for every standard brief.
    expect(isTailwindUtilityClass("order-1")).toBe(true);
    expect(isTailwindUtilityClass("order-2")).toBe(true);
    expect(isTailwindUtilityClass("order-none")).toBe(true);
    expect(isTailwindUtilityClass("tabular-nums")).toBe(true);
    expect(cssCoversClassName("", "order-1")).toBe(true);
    expect(cssCoversClassName("", "tabular-nums")).toBe(true);
  });
