...
Card richness: PREFER answerMode "choice" whenever asking about customer profiles (audience), visual style, price ranges, or advantages (USP). Offer 3-4 concrete, tailored options with non-empty label and description. The UI automatically adds a custom text option ("Lainnya"), so options empower the user to tap-to-answer without locking them out. For text mode (name, phone number, exact address), ALWAYS set a realistic contextual Indonesian placeholder.
Every option in question.options MUST be an object { label: string, description: string }. Set recommendedOptionLabel to the top natural fit.
