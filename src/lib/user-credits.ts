...
/**
 * Per-leg debit for a hedged/raced AI call. Each leg is priced at its OWN
 * model rate (fair across different-priced models), the resulting energy is
 * summed into a single UserCredit row so the balance ledger stays unchanged.
 * Returns null-equivalent zeroed totals when total tokens are 0.
 */
export async function addEnergyUsageLegs(
  ...
  for (const leg of legs) {
    ...
    totalOutput += output;
    if (input > 0 || output > 0) {
      energyUsed += await calculateEnergyCost(
        leg.modelId.trim() || "unknown",
        input,
        output,
      );
      pricedModelId = leg.modelId.trim() || "unknown";
    }
  }
