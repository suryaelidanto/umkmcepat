...
export function createS3StorageProvider(): StorageProvider {
  ...
  const publicBaseUrl = requireEnv("S3_PUBLIC_BASE_URL", { feature: "S3-compatible storage" }).replace(/\/$/, "");

  const client = new S3Client({
    endpoint: getEnv("S3_ENDPOINT") || undefined,
    region: getEnv("S3_REGION", "auto"),
    credentials: {
      accessKeyId: requireEnv("S3_ACCESS_KEY_ID", { feature: "S3-compatible storage" }),
      secretAccessKey: requireEnv("S3_SECRET_ACCESS_KEY", { feature: "S3-compatible storage" }),
    },
  ...
  return {
    ...
    async delete(keys: string[]): Promise<void> {
      await Promise.allSettled(
        keys.filter(Boolean).map((key) => client.send(new DeleteObjectCommand({ Bucket: bucket, Key: key })))
      );
    },
  };
}
