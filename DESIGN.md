...
- **Professional generation:** accepted brief facts compile into a V3 writer contract, immutable blueprint, and one executable V2 kit: `editorial-airy` (01), `menu-led-editorial` (02), `catalog-story` (03), `warm-commerce` (04), or `bold-typographic` (07). A kit must change portable primitives, prompt constraints, source/browser assertions, and the nine-category critic rubric; it is not a business template.
- **Generation protocol:** the writer emits one leading `<design-plan>`, complete `<file>` blocks, and one `<done>` in a single streamed response. It receives no tools. The normal budget is one writer, one final critic, and zero corrections; the attempt maximum is one shared pre-review correction. Platform-owned theme, content, primitives, routes, dependencies, and runtime files remain protected.
- **Quality floor:** source/theme gates reject starter residue, fixed-renderer fingerprints, wrong kit patterns, broken routes/anchors/social links, technical headings, fabricated claims, missing obligations, raw palette literals, placeholder media, and inaccessible contrast. Built candidates pass mobile `390×844` and desktop `1440×1000` browser gates for every route, then receive one calibrated category-complete visual review. Any visual `unknown`, missing evidence, incomplete category set, low confidence, or rating below 3 fails closed; only named deterministic browser assertions may consume the single correction.
- **Composition:** kits define a dominant first-view idea, type roles, palette bounds, rhythm, signature patterns, and anti-patterns. Accepted references calibrate visual behavior only; their identity, facts, prices, contacts, claims, CTA text, and placeholder treatment never enter generated output. Do not collapse unrelated businesses into one universal hero, card grid, route, or default palette.
- **Photo matrix:** `feature.composer_uploads_enabled=false` forces `graphic` or `typographic` composition with zero placeholder files, empty frames, or replacement copy. Enabled plus approved assets uses only `/media/<assetId>`. Enabled without approved assets remains complete and image-free; a future photo-later mode needs a new contract version.
- **Rollout:** professional-static-v3 selection remains blocked until the V4 24-trial benchmark, 50-sample/30-defect calibration, blind readiness and preference gate, CI, and real owner approval pass. Failed or infrastructure-unknown candidates never replace the selected source/build/deployment or mutate the accepted handoff.
- **Taste layer:** `DESIGN_DIRECTIVE` applies on top of contract and recipe constraints — tinted neutrals, one deliberate accent, varied section rhythm, no purple-blue gradients or gradient text, `min-h-dvh` for full-height sections. Generated copy is Indonesian and business-specific, never placeholder or technical component text.
