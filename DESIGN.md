...
- **Generation contract:** accepted brief facts compile into a typed generated-site contract. Landing/marketing generation receives exactly one selected archetype recipe, one matching gold example, and must emit a leading `<design-plan>` plus complete `<file>` blocks in one streamed AI response. `spec.components` is implementation guidance, never visible copy.
- **Quality floor:** deterministic source gates reject starter residue, broken routes/anchors/social links, technical or customer-facing English headings, fabricated claims, missing required content, media-policy violations, and inaccessible semantic token contrast. The compiled theme owns WCAG contrast. Built candidates then pass mobile `390×844` and desktop `1440×1000` browser gates before selection. Risky candidates receive a read-only screenshot critic; at most one targeted visual repair may rerun the full qualification path.
- **Composition:** recipes define hierarchy, rhythm, signature element, forbidden sameness, and media strategy. Gold examples demonstrate structure only; literal leakage is rejected. Prefer varied section surfaces over repeated equal card grids. The scaffold home is a neutral runtime marker, never a shippable visual starting point.
- **Photo matrix:** `feature.composer_uploads_enabled=false` forces graphic/typographic composition with no placeholder or replacement copy. Enabled plus approved assets uses `/media/<assetId>`. Enabled without assets may use coherent replaceable slots only when the selected recipe permits them; the page must remain useful before replacement.
