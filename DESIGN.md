...
- **Reference-calibrated generation:** accepted brief facts compile into a V2 writer contract and one deterministic executable kit: `editorial-airy` (01), `menu-led-editorial` (02), `catalog-story` (03), `warm-commerce` (04), or `bold-typographic` (07). A kit must change portable primitives, prompt constraints, source/browser assertions, and critic rubric; it is not a business template.
- **Generation protocol:** the writer emits one leading `<design-plan>`, complete `<file>` blocks, and one `<done>` in a single streamed response. It receives no tools. The normal budget is one writer, one mandatory visual review, and zero corrections; the attempt maximum is one shared correction. Platform-owned theme, content, primitives, routes, dependencies, and runtime files remain protected.
- **Quality floor:** source/theme gates reject starter residue, fixed-renderer fingerprints, wrong kit patterns, broken routes/anchors/social links, technical headings, fabricated claims, missing obligations, raw palette literals, placeholder media, and inaccessible contrast. Built candidates pass mobile `390×844` and desktop `1440×1000` browser gates, then always receive one reference-aware visual review. Human-only critical/high findings cannot auto-pass; only named deterministic browser assertions may consume the single correction.
...
- **Rollout:** reference-calibrated replacement is disabled by default and remains shadow-only until the 24-trial benchmark and blind owner preference gate pass. Failed candidates never replace the selected source/build/deployment or mutate the accepted handoff.
