...
- Tests for generated AI output enforce truth, safety, accessibility, operability, and review evidence — never pin taste. Never use TDD or unit tests to assert exact AI responses, stochastic wording, model prose, palette hues, fonts, layout structure, card counts, section order, visual-kit/recipe selection, decorative elements, or generated-source snapshots. Writing unit tests for model prose/taste is strictly forbidden. Evaluate rendered quality through calibrated review and evaluation corpora instead.
