...
type Pair = {
  key: string;
  left: GeneratedSiteEvaluationTrialV4;
  right: GeneratedSiteEvaluationTrialV4;
...
async function main(): Promise<void> {
  ...
  const sections = pairs
    .map((pair) => {
      ...
      const leftDesktop = renderEvidence(
        blindDir,
        pair.left.desktopEvidenceRefs,
        "Tampilan desktop A",
      );
      const leftMobile = renderEvidence(
        blindDir,
        pair.left.mobileEvidenceRefs,
        "Tampilan mobile A",
      );
      const rightDesktop = renderEvidence(
        blindDir,
        pair.right.desktopEvidenceRefs,
        "Tampilan desktop B",
      );
      const rightMobile = renderEvidence(
        blindDir,
        pair.right.mobileEvidenceRefs,
        "Tampilan mobile B",
      );
      return `<section class="case" data-key="${escapeHtml(pair.key)}"><h2>Kasus ${escapeHtml(pair.key)}</h2><div class="pair"><figure><figcaption>A</figcaption>${leftDesktop}${leftMobile}</figure><figure><figcaption>B</figcaption>${rightDesktop}${rightMobile}</figure></div><fieldset><legend>Pilihan</legend><label><input type="radio" name="${escapeHtml(pair.key)}" value="left"> A lebih meyakinkan</label><label><input type="radio" name="${escapeHtml(pair.key)}" value="right"> B lebih meyakinkan</label><label><input type="radio" name="${escapeHtml(pair.key)}" value="tie"> Sama kuat</label></fieldset><fieldset><legend>Kesiapan bukti</legend><label><input class="ready-a" type="radio" name="ready-a-${escapeHtml(pair.key)}" value="yes"> A layak dinilai</label><label><input class="ready-a" type="radio" name="ready-a-${escapeHtml(pair.key)}" value="no"> A belum layak</label><label><input class="ready-b" type="radio" name="ready-b-${escapeHtml(pair.key)}" value="yes"> B layak dinilai</label><label><input class="ready-b" type="radio" name="ready-b-${escapeHtml(pair.key)}" value="no"> B belum layak</label></fieldset></section>`;
    })
    .join("\n");
  await writeFile(
...
function pairTrials(trials: GeneratedSiteEvaluationTrialV4[]): Pair[] {
  const keys = new Set(
    trials.filter((trial) => trial.arm === "professional-static-v3").map(keyOf),
  );
  return [...keys].sort().map((key) => {
    const treatment = trials.find(
      (trial) => trial.arm === "professional-static-v3" && keyOf(trial) === key,
    );
