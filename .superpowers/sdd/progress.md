...
Task 12 (calibration/benchmark evidence): BLOCKED — see engine defects below. Two full private calibration runs (20260815123957-ac9403d1, 20260815151442-4a4793da) produced zero usable samples: every treatment trial failed at the writer, so captureCalibrationDefects never ran. Root causes found and fixed, each with a failing test first:
  1. Writer prompt never stated the WriterDesignPlanV3 schema, so the model mirrored the blueprint's vocabulary and parseWriterDesignPlanV3 rejected every plan ("invalid V3 design-plan fields", ledger errorClass=schema on 12/12 trials). Prompt now generates the schema skeleton from the parser's own exported key/enum constants.
  2. compileShadcnTheme hard-failed when a mid-tone background or accent admitted neither black nor white text (e.g. #0077dd). It now resolves the surface deterministically along its own hue; thresholds unchanged.
  3. V3 prompt dropped the concrete module/DOM facts the V2 prompt carries: layout import path and primitive API, usePreviewReady, route export and no default export, literal data-* hooks, external-link rel, min-h-11 touch target, populated site.* fields, text-muted-foreground, eyebrow budget. All are now stated, with populated fields generated from the source gate's own exported list.
  4. compileProfessionalSiteBlueprint gave a synthesized identity section id "hero" even when an accepted section already used it, making data-section-id ambiguous. Ids are now unique per route.
Runner now writes bounded private per-trial diagnostics (progress.jsonl + stdout) so a systematic failure is visible during a run instead of after the corpus.
Remaining blockers, both outside this change: (a) 9Router default-combo has one upstream (deepseek/deepseek-v4-flash) that is intermittently returning 502 "Failed to convert streaming response to JSON" with all accounts locked, and healthy writer calls still take 200s+ TTFT for ~26k output tokens; (b) human calibration labels, adjudication, blind owner readiness/preferences, and owner approval do not exist and cannot be produced by an agent. Release authority stays approved=false; Tasks 13-15 remain unstarted by design.
